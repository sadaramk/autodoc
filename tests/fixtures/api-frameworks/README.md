...
| `aspnet/` | C#: `.csproj` with `Microsoft.NET.Sdk.Web`; top-level `Program.cs` (`WebApplication.CreateBuilder`) as the entry point; `[ApiController]` + `[Route("api/[controller]")]` with the token expanded; `[HttpGet("{id:int}")]`; `[FromQuery(Name = …)]` / `[FromRoute]` / `[FromHeader]` / `[FromBody]`; `[ProducesResponseType(StatusCodes.Status201Created)]`; `NotFound()` / `NoContent()`; `throw new KeyNotFoundException` / `ArgumentException`; `[Authorize(Roles = …)]` / `(Policy = …)` with `[AllowAnonymous]` overriding it; data annotations and `[JsonPropertyName]` on DTOs; an EF Core `DbContext` with `DbSet<T>`, `ToTable` and navigation properties; minimal-API `MapGet("/healthz")` excluded as a probe. |
