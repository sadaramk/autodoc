...
repository, Spring PetClinic, PiggyMetrics, and autodoc's own source.

Docs: [Getting started](https://sadaramk.github.io/autodoc/start.html) ·
...
Commit the book beside the code it documents and gate every push on it:

```yaml
- name: Architecture documentation is current
  run: |
    cargo build --release -p autodoc-cli
    ./target/release/autodoc check . --out docs/architecture
```

`check` exits 1 when regenerating would change the book, or when a citation no
longer matches the code. Committing the book does not invalidate it, and neither
does a commit that changes nothing it describes: a book records the commit it
documents rather than `HEAD`.
